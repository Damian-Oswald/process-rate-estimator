...
df <- data.frame(c("$\\eta\\;\\text{SP}_{\\text{diffusion}}$",
...
![Results of the coefficients for all processes and parameters. The mangitude of the coefficients can be interpreted as the effect size said parameter would have without considering its uncertainty.](scripts/sensitivity-analysis/output/Coefficients.svg){#fig-src width=100%}
:::

### Standardized regression coefficients

What do SRC mean? How to interpret them? How to calculate $\boldsymbol \beta^\ast$?
...
![Share of the total SRC for each process. (1) eta_SP_diffusion, (2) eta_18O_diffusion, (3) SP_nitrification, (4) d18O_nitrification, (5) SP_denitrification, (6) d18O_denitrification, (7) eta_SP_reduction, (8) eta_18O_reduction.](scripts/sensitivity-analysis/output/SRC-importances.svg){#fig-src width=100%}
...
![Barplot showing the Frobenius norm of the covariance matrix ($\| \Sigma \|_F$) of the estimated processes for all depth × column combinations.](scripts/sensitivity-analysis/output/covariance-norm.svg)
...
Once again, we can use the `lmer` function from the `lme4` package to model the process rates as a response of the bulk density as well as a random effect conditional on the depth [@bates2014fitting].
...
m1 <- lme4::lmer(Nitrification ~ BD + (BD | depth), data)
m2 <- lme4::lmer(Denitrification ~ BD + (BD | depth), data)
m3 <- lme4::lmer(Reduction ~ BD + (BD | depth), data)
