...
Sensitivity analysis is a method used to determine how different values of model parameters impact a particular model output value under a given set of assumptions [@trucano2006calibration]. Sometimes, sensitivity analyses are also described as investigating the models sensitivity to its inputs and not its parameters [@borgonovo2016sensitivity]. For the process rate estimator, the sensitivity of the PRE to changes to the most important parameters such as the isotope end members or the bulk density are investigated -- however, the model inputs, i.e. the moisture, N~2~O, SP and δ^18^O measurements are assumed to be true.

<!-- [FOOTNOTE]: Of course, one could start including more and more model parameters or pre-processing variables to the sensitivity analysis, such as physical parameters used in the pre-processing, the bandwith parameter used for the smoothing, *et cetera*. -->
...
The prior distribution of the parameters was informed by the relevant literature and displayed in [table @tbl-sensitivity].
...
| `SP_nitrification`     | $\text{SP}_\text{nit.}$         | 35        | @decock2013potential    | $\sim \mathcal{U}(32, 38.7)$     | @denk2017nitrogen |
| `d18O_nitrification`   | $\delta\ce{^{18}O_\text{nit.}}$ | 23.5      | @lewicka2017quantifying | $\sim \mathcal{N}(23.5, 3^2)$    |                   |
| `SP_denitrification`   | $\text{SP}_\text{den.}$         | -4.85     | @decock2013potential    | $\sim \mathcal{U}(-13.6, 3.7)$   | @denk2017nitrogen |
| `d18O_denitrification` | $\delta\ce{^{18}O_\text{den.}}$ | 9.3       | @lewicka2017quantifying | $\sim \mathcal{U}(2.5, 13.4)$    |                   |
| `eta_SP_reduction`     | $\eta\:\text{SP}_\text{red.}$   | -5.9      | @denk2017nitrogen       | $\sim \mathcal{U}(-8.2, -2.9)$   | @denk2017nitrogen |
| `eta_18O_reduction`    | $\eta\ce{^{18}O_\text{red.}}$   | -15.4     | @lewicka2017quantifying | $\sim \mathcal{U}(-25.1, -5.1)$  |                   |
