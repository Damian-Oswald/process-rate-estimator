...
#' @param derivatives Derivatives of `N2O`, `SP` and `d18O` with respect to time.
#' @param fluxes Values of `N2O`, `SP` and `d18O` as well as their fluxes from other increments.
#' 
...
stateEquations <- function(x = c(N2Onit = NA, N2Oden = NA, N2Ored = NA),
   ...
   with(fluxes, {
      ...
      f[2] <- (F_top_in*(SP_top - e[3] - SP) + F_bottom_in*(SP_bottom - e[3] - SP) + x[1]*(e[1]-SP) + 
                  x[2]*(e[2]-SP)-(e[3]*F_out + e[4]*x[3]))/N2O - SP_derivative
      f[3] <- (F_top_in*(d18O_top - e[7] - d18O) + F_bottom_in*(d18O_bottom - e[7] - d18O) + 
                  x[1]*(e[5]-d18O) + x[2]*(e[6]-d18O)-(e[7]*F_out + e[8]*x[3]))/N2O - d18O_derivative
      return(f)
   })
}