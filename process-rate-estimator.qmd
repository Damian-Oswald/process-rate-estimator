...
```{r}
#| label: tbl-hyperparameters-1
#| tbl-cap: List of all the best working hyperparameters (bandwidth) for every combination of depth, column for the `gN2ONha` variable. The search range was $[5,100]$.
#| echo: false
load("resources/hyperparameters.RData")
knitr::kable(signif(hyperparameters[,,1],3))
```
```{r}
#| label: tbl-hyperparameters-2
#| tbl-cap: List of all the best working hyperparameters (bandwidth) for every combination of depth, column for the `SP` variable. The search range was $[5,100]$.
#| echo: false
load("resources/hyperparameters.RData")
knitr::kable(signif(hyperparameters[,,2],3))
```
```{r}
#| label: tbl-hyperparameters-3
#| tbl-cap: List of all the best working hyperparameters (bandwidth) for every combination of depth, column for the `d18O` variable. The search range was $[5,100]$.
#| echo: false
load("resources/hyperparameters.RData")
knitr::kable(signif(hyperparameters[,,3],3))
```

![Visualization of the optimal hyperparameter size by depth, column and variable.](results/hyperparameters.svg){#fig-hyperparameters}
...
| `concNO3N`         | NO~3~-N concentration       | Nitrate nitrogen concentration ([NO~3~] = [NO~3~-N] * 4.43).|
| `NO3N_ha`          |                             |                                                             |
| `corrected.N2O`    |                             |                                                             |
| `corrected.CO2`    |                             |                                                             |
| `mgN2ONm3`         |                             |                                                             |
